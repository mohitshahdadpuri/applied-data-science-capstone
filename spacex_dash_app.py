...
def update_graph(entered_site, payload_slider):
    ...
    if entered_site == 'ALL':
        ...
        filtered_df0 = spacex_df[spacex_df['Launch Site']==entered_site]
        filtered_df = filtered_df0[(filtered_df0['Payload Mass (kg)']>=payload_slider[0]) & (spacex_df['Payload Mass (kg)']<=payload_slider[1])]
        fig = px.scatter(filtered_df, x = 'Payload Mass (kg)', y='class', color = 'Booster Version Category')
        return fig        
        # return the outcomes piechart for a selected site
